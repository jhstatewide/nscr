 ...
 * - Complete container push workflows
...
class PushFlowIntegrationTest {
    ...
    fun `complete push flow should not garbage collect referenced blobs`() {
        ...
        // Create realistic layer content that simulates actual container layers
        ...
        // Step 2: Create a manifest that references all 5 layers (like container runtimes do)
    ...
    fun `docker push workflow should preserve all layers after garbage collection`() {
        ...
        // 1. Container push nscr.lan:7000/dockage/mailcatcher (5 layers pushed)
        ...
        for (i in 1..5) {
            val sessionId = SessionID(UUID.randomUUID().toString())
            val layerContent = "Container layer content for ${layerNames[i-1]}".toByteArray()
    ...
    private fun createRealisticManifestJson(layerDigests: List<Digest>): String {
        // Create a realistic container manifest that references the layers
        // This simulates what container runtimes actually create when pushing an image
